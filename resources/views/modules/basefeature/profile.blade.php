...
@section('page_subtitle', 'Kelola identitas akun, kontak pribadi, avatar, dan keamanan login tenant')
...
@section('content')
    @php
        ...
        $workspaceName = $tenantSetting->brand_name ?? tenant('name') ?? tenant('id') ?? config('app.name', 'AirCloud');
    @endphp
    ...
    <div class="page-grid">
        ...
        @if (! $profileSchemaReady)
            <div class="alert alert-danger">
                Kolom `phone_number` dan `avatar_path` di database tenant belum siap. Nama, email, dan password tetap bisa diubah, tapi jalankan migrasi tenant terbaru dulu untuk menyimpan nomor HP dan avatar.
            </div>
        @endif
        ...
        <section class="hero-card">
            <div class="profile-hero-copy">
                <span class="hero-badge"><i class="fas fa-id-badge"></i> Profil Pengguna</span>
                <h2>{{ $displayName }}</h2>
                <p>Update biodata akun tenant, jabatan operasional, dan ubah password tanpa perlu masuk ke menu manajemen user.</p>
            </div>
            ...
            <div class="hero-meta">
                ...
                <div class="hero-meta-card">
                    <span>Jabatan</span>
                    <strong>{{ $jobTitle !== '' ? $jobTitle : '-' }}</strong>
                </div>
                <div class="hero-meta-card">
                    <span>Workspace</span>
            ...
            <div class="dashboard-card">
                <div class="card-head">
                    <div>
                        ...
                        <p class="card-subtitle">Nama, email, nomor HP, dan avatar yang tampil di workspace tenant.</p>
                    </div>
                </div>

                <form method="POST" action="{{ route('tenant.profile.update') }}" enctype="multipart/form-data" class="form-stack">
                    @csrf
                    @method('PATCH')
                    ...
                    <div class="profile-form-grid">
                        <div class="form-block">
                            ...
                            <label class="field-label profile-field-top" for="profile_job_title">Jabatan</label>
                            @if ($jobTitleSchemaReady)
                                @php
                                ...
                                @if ($jobTitleCatalogActive && $jobTitleCatalogOptions->count() > 0)
                                    <select id="profile_job_title" name="job_title">
                                ...
                                @else
                                    ...
                                    >
                                @endif
                                <p class="profile-help">Isi sesuai posisi kerja nyata di tenant, misalnya admin cabang, kolektor, kasir, atau supervisor lapangan.</p>
                            @else
                                <input
                                    id="profile_job_title"
                                    type="text"
                                    class="field"
                                    value="Jalankan migrasi tenant terbaru dulu"
                                    disabled
                                >
                            @endif
                            ...
                            >
                            <p class="profile-help">Boleh kosong. Simpan format nomor yang gampang dibaca operator, misalnya `0812 3456 7890` atau `+62 812 3456 7890`.</p>
                        </div>
                        ...
                        <div class="form-block">
                            ...
                            <div class="avatar-panel">
                                ...
                                <div class="avatar-panel-copy">
                                    ...
                                    <span>{{ $jobTitle !== '' ? $jobTitle.' · '.$roleLabel : $roleLabel.' tenant' }}</span>
                                </div>
                            </div>
                ...
                <div class="dashboard-card">
                    <div class="card-head">
                        <div>
                            ...
                            <p class="card-subtitle">Flow dipisah biar update password tetap aman dan jelas.</p>
                        </div>
                    </div>

                    <form method="POST" action="{{ route('tenant.profile.password.update') }}" class="form-stack">
                        @csrf
                        @method('PATCH')
                ...
                <div class="dashboard-card">
                    <div class="card-head">
                        <div>
                            ...
                            <p class="card-subtitle">Snapshot cepat identitas akun yang sedang aktif dipakai.</p>
                        </div>
                    </div>
